...
/**
 * ============================================================================
 * herramientas_requisiciones.php - Requisiciones de compra de herramientas
 * ============================================================================
 * Espejo del módulo de refacciones, adaptado a herramientas.
 * ============================================================================
 */
...
if (es_post() && $puede_gestionar && $tabla_ok) {
    if (!csrf_valido(input('_csrf'))) {
        ...
        try {
            ...
            registrar_auditoria('crear_requisicion', 'herramienta_requisiciones', $req_id, 'Requisición de herramientas creada');
            flash_set('exito', 'Requisición creada. Agrega los renglones que necesites.');
            header('Location: ' . url('herramienta_requisicion_ver.php?id=' . $req_id));
            exit;
        } catch (Throwable $e) {
...
$requisiciones = $tabla_ok ? hreq_listar([
...
$titulo_pagina = 'Requisiciones de herramientas';
...
<div class="animate-fade-in space-y-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-display text-2xl font-extrabold text-zinc-900 flex items-center gap-2">
                    ...
                    Requisiciones de herramientas
                </h2>
                <p class="text-xs text-zinc-500 mt-0.5">Solicitudes de compra de herramientas · formato 0069-FRM</p>
            </div>
        </div>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800 flex items-start gap-2">
        ...
        <span>Falta crear las tablas. Corre <strong>migracion_requisiciones_herramientas.sql</strong> en phpMyAdmin para activar este módulo.</span>
    </div>
    <?php endif; ?>
    ...
    <form method="GET" class="bg-white rounded-xl border border-zinc-200 shadow-sm p-3 flex flex-wrap gap-2 items-end">
        ...
        <a href="<?= url('herramientas_requisiciones.php') ?>" class="px-3 py-2 rounded-lg border border-zinc-300 text-sm text-zinc-600 hover:bg-zinc-50">Limpiar</a>
        <?php endif; ?>
    </form>
    ...
    <div class="bg-white rounded-xl border border-zinc-200 py-16 text-center">
        ...
        <p class="text-sm text-zinc-500 mt-1">Crea la primera para empezar a solicitar compras de herramientas.</p>
    </div>
    <?php else: ?>
                ...
                <tbody class="divide-y divide-zinc-100">
                    ...
                    <tr class="hover:bg-zinc-50">
                        <td class="px-4 py-2.5">
                            <a href="<?= url('herramienta_requisicion_ver.php?id=' . $r['id']) ?>"
                        ...
                        <td class="px-4 py-2.5 text-right whitespace-nowrap">
                            <a href="<?= url('herramienta_requisicion_imprimir.php?id=' . $r['id']) ?>" target="_blank"
