...
/**
 * ============================================================================
 * herramienta_requisicion_ver.php - Detalle de una requisición de herramientas
 * ============================================================================
 * Renglones (texto libre o herramienta del catálogo como modelo), status por
 * renglón, autorización (admin), recepción opcional (alta en catálogo) y PDF.
 * ============================================================================
 */
require_once __DIR__ . '/config/db.php';
require_once __DIR__ . '/config/auth.php';
require_once __DIR__ . '/config/helpers.php';
require_once __DIR__ . '/config/herramientas_requisiciones_helpers.php';

requerir_login();
$u = usuario_actual();

if (!hreq_disponible()) {
    flash_set('error', 'Falta correr migracion_requisiciones_herramientas.sql.');
...
if (!$req) {
    ...
    header('Location: ' . url('herramientas_requisiciones.php'));
    exit;
}
    ...
    if (!csrf_valido(input('_csrf'))) {
        ...
        try {
            if ($op === 'item_agregar' && $editable) {
                hreq_item_agregar($id, [
                    'herramienta_id' => input('herramienta_id', '') !== '' ? (int) input('herramienta_id') : null,
                ...
                hreq_item_actualizar((int) input('item_id', 0), [
                    'descripcion' => input('descripcion', ''),
                    'cantidad'    => input('cantidad', 0),
                    'unidad'      => input('unidad', ''),
                    'area_id'     => input('area_id', '') !== '' ? (int) input('area_id') : null,
                ...
                $it = db_one("SELECT * FROM herramienta_requisicion_items WHERE id = :i AND requisicion_id = :r",
                    ['i' => $item_id, 'r' => $id]);
                if ($it) {
                    hreq_item_actualizar($item_id, [
                ...
                $res = hreq_item_recibir((int) input('item_id', 0), [
                    'cantidad'          => input('cantidad', 0),
                    'costo_unitario'    => input('costo_unitario', ''),
                    'crear_herramienta' => input('crear_herramienta') ? 1 : 0,
                    'tipo'              => input('tipo', ''),
                    'ubicacion'         => input('ubicacion', ''),
                ], (int) $u['id']);
                registrar_auditoria('recibir_requisicion_item', 'herramienta_requisicion_items', (int) input('item_id', 0), $req['folio']);
                flash_set('exito', $res['creadas'] > 0
                    ? ('Recepción registrada. Se dieron de alta ' . $res['creadas'] . ' herramienta(s) en el catálogo.')
                    : 'Recepción registrada (sin crear herramientas).');

            } elseif ($op === 'item_eliminar' && $editable) {
                hreq_item_eliminar((int) input('item_id', 0));
                flash_set('exito', 'Renglón eliminado.');

            } elseif ($op === 'cabecera' && $editable) {
                hreq_actualizar($id, trim((string) input('fecha', '')), trim((string) input('notas', '')), (string) input('razon_social', ''));
                ...
                if ($nuevo === 'autorizada' && !$es_admin) {
                    ...
                    hreq_cambiar_estado($id, $nuevo, (int) $u['id']);
                    registrar_auditoria('estado_requisicion', 'herramienta_requisiciones', $id, "Estado: {$nuevo}");
                    flash_set('exito', 'Estado actualizado.');
                }

            } elseif ($op === 'eliminar' && $es_admin) {
                hreq_eliminar($id);
                registrar_auditoria('eliminar_requisicion', 'herramienta_requisiciones', $id, $req['folio']);
            ...
            if (empty($errores)) {
                header('Location: ' . url('herramienta_requisicion_ver.php?id=' . $id));
                exit;
            }
        } catch (Throwable $e) {
...
$editable = $puede_gestionar && in_array($req['estado'], ['borrador', 'enviada'], true);
$puede_recibir = $puede_gestionar && in_array($req['estado'], ['enviada', 'autorizada', 'cerrada'], true);
$pendientes    = hreq_items_pendientes($id);

$catalogo  = db_all("SELECT id, codigo, nombre FROM herramientas WHERE activo = 1 ORDER BY nombre");
$areas     = db_all("SELECT id, nombre FROM areas WHERE activo = 1 ORDER BY nombre");
$unidades  = ['pieza' => 'Pieza', 'juego' => 'Juego', 'par' => 'Par', 'kit' => 'Kit', 'caja' => 'Caja', 'metro' => 'Metro'];
...
<div class="animate-fade-in space-y-4">
    ...
    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
        <div class="flex items-center gap-3">
            <a href="<?= url('herramientas_requisiciones.php') ?>" class="p-2 rounded-lg hover:bg-zinc-100 text-zinc-500">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            <a href="<?= url('herramienta_requisicion_imprimir.php?id=' . $id . '&formato=controlado') ?>" target="_blank"
            ...
            <a href="<?= url('herramienta_requisicion_imprimir.php?id=' . $id . '&formato=simple') ?>" target="_blank"
               class="px-3 py-2 rounded-lg border border-zinc-300 bg-white text-sm font-medium text-zinc-700 hover:bg-zinc-50 flex items-center gap-1.5">
                ...
                <tbody class="divide-y divide-zinc-100">
                    ...
                    <tr class="hover:bg-zinc-50">
                        ...
                        <td class="px-4 py-2.5">
                            ...
                            <?php if ($it['herramienta_codigo']): ?>
                            <div class="text-[10px] text-zinc-400 font-mono">Modelo: <?= e($it['herramienta_codigo']) ?></div>
                            <?php endif; ?>
                            <?php if ($it['notas']): ?>
        ...
        <div class="border-t border-zinc-200 bg-zinc-50 p-4">
            <form method="POST" class="grid grid-cols-1 sm:grid-cols-12 gap-2 items-end" id="form-item">
                ...
                <div class="sm:col-span-4">
                    <label class="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Modelo del catálogo (opcional)</label>
                    <select name="herramienta_id" id="sel_herramienta" onchange="autollenar()"
                ...
                <div class="sm:col-span-4">
                    ...
                    <input type="text" name="descripcion" id="inp_desc" required maxlength="300"
                           class="w-full px-3 py-2 rounded-lg border border-zinc-300 text-sm" placeholder="Qué herramienta se necesita">
                </div>
                <div class="sm:col-span-1">
                    <label class="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Cant. <span class="text-red-500">*</span></label>
                    <input type="number" name="cantidad" required step="1" min="1" value="1"
                           class="w-full px-3 py-2 rounded-lg border border-zinc-300 text-sm">
                </div>
                <div class="sm:col-span-1">
                    ...
                    <select name="unidad" id="sel_unidad" class="w-full px-3 py-2 rounded-lg border border-zinc-300 bg-white text-sm">
                        ...
                        <?php foreach ($unidades as $k => $lbl): if ($k === 'pieza') continue; ?>
        ...
        <form method="POST" class="space-y-3">
            ...
            <div class="grid grid-cols-3 gap-3">
                <div>
                    ...
                    <input type="number" name="cantidad" id="e_cant" required step="1" min="1"
                           class="w-full px-3 py-2 rounded-lg border border-zinc-300 text-sm">
                </div>
                <div>
        ...
        <form method="POST" class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="number" name="cantidad" id="r_cant" required step="1" min="1"
                           class="w-full px-3 py-2 rounded-lg border border-zinc-300 text-sm">
            ...
            <!-- Alta opcional en catálogo -->
            <div id="r_alta" class="bg-emerald-50 border border-emerald-200 rounded-lg p-3 space-y-2">
                <label class="flex items-start gap-2 cursor-pointer">
                    <input type="checkbox" name="crear_herramienta" id="r_crear" value="1" onchange="toggleAlta()"
                           class="mt-0.5 w-4 h-4 rounded border-zinc-300 text-bacal-700 focus:ring-bacal-500">
                    <span class="text-xs font-semibold text-zinc-800">Dar de alta la(s) herramienta(s) en el catálogo (una por pieza, con código autogenerado)</span>
                </label>
                <div id="r_alta_campos" class="hidden grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-[10px] font-bold text-zinc-600 uppercase mb-1">Tipo (opcional)</label>
                        <input type="text" name="tipo" maxlength="80" placeholder="Eléctrica, manual, medición…"
                               class="w-full px-3 py-2 rounded-lg border border-zinc-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-zinc-600 uppercase mb-1">Ubicación (opcional)</label>
                        <input type="text" name="ubicacion" maxlength="150" placeholder="Taller, Anaquel B-2…"
                               class="w-full px-3 py-2 rounded-lg border border-zinc-300 text-sm">
                    </div>
                </div>
                <p class="text-[10px] text-emerald-700">Si lo dejas sin marcar, solo se registra la compra (no crea herramientas en el catálogo).</p>
            </div>
...
<script>
function autollenar() {
    var sel = document.getElementById('sel_herramienta');
    if (!sel) return;
...
function editarItem(d) {
    ...
    document.getElementById('e_unidad').value  = d.unidad || 'pieza';
